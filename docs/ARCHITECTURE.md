...
- Wide desktop uses one 72-pixel integrated bar. Constrained desktop keeps all four clocks on a compact second row, while ordinary tablet and mobile workspaces use a two-by-two grid without hiding a zone. On the phone-width SygSphere route, the same four clocks use one readable horizontal strip with Pacific and Mountain system time first; the remaining zones stay reachable by a direct horizontal swipe so messaging and its composer retain the usable viewport.
