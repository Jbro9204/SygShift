...
-- Keep employee clock-in eligibility authoritative at the database boundary.
-- Early attempts return a structured domain rejection so every UI surface can
-- render the same required acknowledgment without creating payable time.
...
begin
  ...
  if target_kind = 'clock_in' then
    ...
    if server_now < clock_in_eligible_at then
      -- A short operational deduplication window records the blocked attempt
      -- without allowing rapid clicks to flood the append-only audit history.
      if not exists (
        select 1
      ...
      return jsonb_build_object(
        ...
        'clockInEligibleAt', clock_in_eligible_at,
        'shiftDate', (selected_shift.starts_at at time zone selected_shift.time_zone)::date,
...
comment on function public.record_time_event(public.time_event_kind, uuid, timestamptz, text) is
  'Records authorized employee time using trusted server time; early assigned-shift attempts return EARLY_CLOCK_IN_BLOCKED without creating payable time.';
