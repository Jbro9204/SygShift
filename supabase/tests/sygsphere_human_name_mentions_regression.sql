...
begin
  ...
  -- Exercise the same completed-security state required by SygSphere. This keeps
  -- the send-path regression valid for employees whose role requires MFA.
  perform set_config('request.jwt.claims',jsonb_build_object('sub',actor_auth,'role','authenticated','aal','aal2')::text,true);
  ...
  created_message_id := (result->>'id')::uuid;
  ...
  assert exists(select 1 from private.sygsphere_mentions stored where stored.message_id=created_message_id and stored.employee_id=recipient_id and stored.label=selected_label),
