...
function waitForAlarmInterval(signal: AbortSignal) {
  return new Promise<void>((resolve) => {
    ...
    const timer = window.setTimeout(finish, alarmIntervalMs)
    function finish() {
...
export function SygTasksAlarmHost({ employeeId }: { employeeId: string }) {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => {
    ...
    return preferences.alarm && !preferences.muted && preferences.volume > 0
  })
  ...
  useEffect(() => {
    const update = () => {
      const preferences = getSoundPreferences()
      setSoundEnabled(preferences.alarm && !preferences.muted && preferences.volume > 0)
    }
    ...
    void runWithSingleTabLease(employeeId, controller.signal, async () => {
      while (!controller.signal.aborted) {
        if (document.visibilityState === 'visible') {
          const played = await playSound('alarm')
          if (!played && !controller.signal.aborted) setAudioBlocked(true)
        }
  ...
  const soundStatus = useMemo(() => {
    ...
    return 'Repeating about every 8 seconds until stopped, snoozed, opened, or completed.'
  ...
  return <aside className="sygtasks-alarm-host" aria-label="Active task alarm" aria-live="assertive">
    <article className={`sygtasks-alarm-host__card sygtasks-alarm-host__card--${activeAlarm.priority}`}>
      ...
      <div className="sygtasks-alarm-host__actions">
        ...
        <Link className="sygtasks-button sygtasks-button--secondary" to={sygTaskPath(activeAlarm.boardId, activeAlarm.taskId)} onClick={() => silenceLocally(activeAlarm.occurrenceId)}>Open task <ExternalLink aria-hidden="true" size={15} /></Link>
