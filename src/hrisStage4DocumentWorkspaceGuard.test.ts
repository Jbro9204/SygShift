...
describe('HRIS Stage 4 protected document workspace', () => {
  ...
  it('uses a compact inventory and deliberate modal workflows', () => {
    ...
    expect(page).toContain('Open a PDF')
    expect(page).toContain('DocumentWorkbench')
    expect(page).toContain('getHrDocumentBlob(document.id, action)')
    expect(page).toContain('document-access-progress')
    expect(page).not.toContain('Explain why this document is needed.')
    expect(worker).toContain('Authorized ${action} from Document Studio.')
  })
})
