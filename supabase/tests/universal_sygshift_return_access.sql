...
begin
  ...
  if eligible_guard_auth_user_id is null or mfa_required_auth_user_id is null then
    raise exception 'Reciprocal launch verification requires eligible Guard and MFA-required subjects.';
  end if;
  ...
  if not (
    array['apps.sygilant.access', 'apps.sygshift.access']::text[]
      <@ public.get_effective_permissions()
  ) then
    raise exception 'An eligible canonical Guard did not receive both reciprocal launcher permissions at AAL1.';
  end if;
  ...
  if exists (
    ...
    where catalog.requires_mfa
      and projected.permission_code not in ('apps.sygilant.access', 'apps.sygshift.access')
  ) then
    raise exception 'The Guard reciprocal exception projected an unrelated MFA-sensitive permission.';
  end if;
  ...
  if session_context_record.role <> 'guard'
     ...
     or not (
       array['apps.sygilant.access', 'apps.sygshift.access']::text[]
         <@ session_context_record.permissions
     ) then
    raise exception 'The Guard AAL1 session context did not expose both approved reciprocal launchers.';
  end if;
  ...
  if public.get_effective_permissions()
       && array['apps.sygilant.access', 'apps.sygshift.access']::text[] then
    raise exception 'An MFA-required employee received a platform launcher permission at AAL1.';
  end if;
  ...
  if not (
    ...
    raise exception 'An approved MFA-required employee lost reciprocal launch access after MFA.';
  end if;
end
$$;
