...
function MessageActionPopover({ label, trigger, panelClassName, children, openOnHover = false }: { label: string; trigger: ReactNode; panelClassName: string; children: ReactNode; openOnHover?: boolean }) {
  ...
  const openPopover = () => {
    if (!openOnHover || !detailsRef.current) return
  ...
  return <details ref={detailsRef} onToggle={(event) => setOpen(event.currentTarget.open)} onPointerEnter={(event) => {
    if (event.pointerType === 'mouse' || event.pointerType === 'pen') openPopover()
  }} onPointerLeave={() => {
    if (openOnHover && detailsRef.current && !detailsRef.current.contains(document.activeElement)) close()
  }} onFocusCapture={(event) => {
    if (event.target instanceof HTMLElement && event.target.matches(':focus-visible')) openPopover()
  }} onBlurCapture={(event) => {
    if (openOnHover && !event.currentTarget.contains(event.relatedTarget)) close()
  }} onKeyDown={(event) => {
...
function Avatar({ name, photoPath, presence }: { name: string; photoPath?: string | null; presence?: PlatformPresenceStatus }) {
  ...
  return <span className="sphere-avatar" aria-hidden="true">{url ? <img src={url} alt="" /> : name.split(/\s+/).slice(0, 2).map((part) => part[0]).join('')}{presence ? <i className={`sphere-avatar__presence sphere-avatar__presence--${presence}`} title={platformPresenceLabels[presence]} /> : null}</span>
}
      ...
      <nav className="sphere-conversations" aria-label="SygSphere conversations">
        ...
        <div className="sphere-conversation-list">{inbox.isPending ? <p role="status">Loading your conversations…</p> : null}
          {(['Favorites', 'Channels', 'Messages'] as const).map((section) => {
            ...
            return items.length ? <section key={section}><h2>{section}</h2>{items.map((item) => <button type="button" key={item.id} className={`sphere-conversation ${conversationId === item.id ? 'is-active' : ''}`} onClick={() => openConversation(item.id)} aria-current={conversationId === item.id ? 'page' : undefined} title={item.kind === 'direct' && item.avatar ? `${item.name} · ${platformPresenceLabels[item.avatar.presence]}` : item.name}>
              {item.kind === 'channel' ? <span className="sphere-avatar"><Hash size={20} /></span> : <Avatar name={item.name} photoPath={item.avatar?.photoPath} presence={item.avatar?.presence} />}<span><strong>{item.name}</strong>{item.kind === 'direct' && item.avatar ? <span className="visually-hidden">Presence: {platformPresenceLabels[item.avatar.presence]}. </span> : null}<small>{item.archived ? 'Archived · ' : ''}{item.latest?.body || 'Start the conversation'}</small></span>{item.unread > 0 ? <b className="sphere-badge">{item.unread > 99 ? '99+' : item.unread}</b> : item.muted ? <BellOff size={14} /> : null}
            </button>)}</section> : null
          })}
    ...
    <Avatar name={message.authorName} photoPath={author?.photoPath} presence={author?.presence} /><div className="sphere-message__content"><header><strong>{message.authorName}</strong><time dateTime={message.createdAt}>{messageTime(message.createdAt)}</time>{message.editedAt && !message.deleted ? <small>Edited</small> : null}{message.pinned && !message.deleted ? <small>📌 Pinned</small> : null}</header>
      {message.deleted ? <p className="sphere-hint">This message was deleted.</p> : <><MessageBody text={message.body} mentions={message.mentions} employeeId={employeeId} />
        ...
        <div className="sphere-message-actions">{!hideReply ? <button type="button" onClick={() => onThread(message)}><MessageCircle size={14} />{message.replyCount ? `${message.replyCount} ${message.replyCount === 1 ? 'reply' : 'replies'}` : 'Reply'}{message.unreadReplies > 0 ? <span className="sphere-unread-dot" aria-label="Unread replies" /> : null}</button> : null}
          ...
          {message.authorId === employeeId ? message.readBy.length ? <MessageActionPopover label={`Read by ${message.readBy.length}. Show readers.`} trigger={<><Check size={13} />Read by {message.readBy.length}</>} panelClassName="sphere-read-panel" openOnHover><ul>{message.readBy.map((person) => <li key={person.id}><Avatar name={person.name} photoPath={person.photoPath} /><span><strong>{person.name}</strong><time dateTime={person.readAt}>{messageTime(person.readAt)}</time></span></li>)}</ul></MessageActionPopover> : <span className="sphere-delivery" title="Saved securely to SygSphere"><Check size={13} />Sent</span> : null}
        </div></>}
