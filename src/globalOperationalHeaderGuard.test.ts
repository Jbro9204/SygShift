...
describe('global operational time header guardrails', () => {
  it('extends the single authenticated shell and preserves its account and alert systems', () => {
    ...
    expect(shell).toContain("timeZone={personalDisplayTimeZone(sessionContext?.timeZone ?? 'America/Denver')}")
  ...
  it('uses one cleaned-up timer and the viewer system time zone', () => {
    expect(header.match(/window\.setInterval/g)).toHaveLength(1)
    expect(header).toContain('window.clearInterval(interval)')
    expect(shell).toContain('personalDisplayTimeZone')
    expect(header).toContain('timeZone?: string | null')
    expect(header).toContain('System time for')
    expect(header).toContain('<CalendarDays')
    expect(header).not.toContain('aria-live')
    expect(header).not.toContain('AnalogClock')
    expect(header).not.toContain('OPERATIONAL_TIME_ZONES')
  ...
  it('keeps one readable system-time display in the integrated top bar and places an inset alert below', () => {
    expect(header).toContain('className="user-system-time"')
    expect(header.indexOf('className="topbar"')).toBeLessThan(header.indexOf('user-system-time'))
    expect(header).toContain('className="user-system-time__time"')
    expect(header).toContain('className="user-system-time__zone"')
    expect(css).toContain('.user-system-time {')
    expect(css).toContain('.user-system-time__time {')
    expect(css).toContain('.user-system-time__zone {')
    expect(css).not.toContain('.operational-clock')
    expect(themeCss).not.toContain('.operational-clock')
