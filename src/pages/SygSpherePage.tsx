...
function SphereComposer({ employeeId, conversationId, parentId, members }: { employeeId: string; conversationId: string; parentId: string | null; members: SpherePerson[] }) {
  ...
  const upload = useMutation({ mutationFn: async (pendingUploadId?: string) => pendingUploadId
    ? sphereCompleteUpload(pendingUploadId, (percentage, stage) => { setUploadProgress(percentage); setUploadStage(stage) })
    : file ? sphereUpload(file.file, file.id, conversationId, parentId, (percentage, stage) => { setUploadProgress(percentage); setUploadStage(stage) }) : null, onSuccess: async (result) => {
  ...
  return <form className="sphere-composer" onSubmit={submit}>
    ...
    {file ? <ModalDialog title="Share a file" description="The file will be available to everyone in this conversation." className="sphere-modal" busy={upload.isPending || retryScan.isPending} busyLabel={uploadStage === 'scanning' ? 'Finishing your file…' : `Uploading… ${uploadProgress}%`} onClose={() => setFile(null)}><div className="sphere-form"><strong>{file.file.name}</strong><p>{(file.file.size / 1048576).toFixed(2)} MB · Maximum 100 MB</p><p className="sphere-hint">PDF, images, text, DOCX and XLSX are supported through 25 MB. JPEG, PNG and WebP images can be as large as 100 MB. Your draft and selected file stay in place if the connection is interrupted.</p>{upload.isPending ? <><progress aria-label="File upload progress" max="100" value={uploadProgress}>{uploadProgress}%</progress><p className="sphere-hint">{uploadStage === 'scanning' ? 'Upload complete. Making the file available…' : 'Uploading your file…'}</p></> : null}<ErrorNotice error={upload.error || retryScan.error} />{upload.error instanceof SphereUploadError && upload.error.requestReference ? <p className="sphere-reference">Reference ID: <code>{upload.error.requestReference}</code></p> : null}<footer><button type="button" disabled={upload.isPending || retryScan.isPending} onClick={() => setFile(null)}>Cancel</button>{retryableUpload ? <button type="button" className="sphere-primary" disabled={retryScan.isPending} onClick={() => retryScan.mutate(retryableUpload.uploadId!)}>{retryScan.isPending ? 'Trying again…' : 'Try again'}</button> : <button type="button" className="sphere-primary" disabled={upload.isPending || retryScan.isPending || file.file.size > 104857600 || file.file.size < 1} onClick={() => upload.mutate(pendingCompletion?.uploadId)}>{upload.isPending && upload.variables ? 'Finishing…' : pendingCompletion ? 'Finish upload' : upload.isError ? 'Retry upload' : 'Share file'}</button>}</footer></div></ModalDialog> : null}
