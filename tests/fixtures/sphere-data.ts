...
const people: SpherePerson[] = [{ id: a, name: 'Mara Chen', username: 'mchen', role: 'guard', presence: 'available', active: true, owner: true }, { id: b, name: 'Devon Ruiz', username: 'druiz', role: 'dispatcher', presence: 'available', active: true, owner: false }, { id: c, name: 'Casey Morgan', username: 'cmorgan', role: 'supervisor', presence: 'away', active: true, owner: false }]
type State = { conversations: SphereConversation[]; messages: SphereMessage[]; reads: Record<string, string[]>; soundEnabled: boolean }
function state(): State { return JSON.parse(localStorage.getItem(key) || JSON.stringify({ conversations: [{ id: conversationId, kind: 'channel', name: 'Operations', description: 'Daily handoffs and coordination.', archived: false, owner: actor === a, muted: false, favorite: true, updatedAt: new Date().toISOString(), unread: 0, latest: null }], messages: [], reads: {}, soundEnabled: false })) }
...
function request(action: string, input: Record<string, unknown>) {
  ...
  if (action === 'create') { const id = crypto.randomUUID(); value.conversations.push({ id, name: String(input.name || people.find((person) => (input.members as string[]).includes(person.id))?.name), kind: input.kind as SphereConversation['kind'], description: '', owner: true, favorite: false, muted: false, archived: false, updatedAt: new Date().toISOString(), unread: 0, latest: null }); write(value); return { id } }
  if (action === 'send') { if (fail) { fail = false; throw new Error('Connection interrupted. Please retry.') } const message = addMessage(value, String(input.body), actor, cid, input.parentId ? String(input.parentId) : null); write(value); return decorate(value, message) }
...
export function getSupabaseClient() {
  return { auth: { getSession: async () => ({ data: { session: { access_token: 'fixture-only', user: { id: actor } } } }) }, channel: () => {
    let listener = () => {}
  ...
  }, removeChannel: async (channel: { close: () => void }) => channel.close(), rpc: async (name: string, args: { action: string; input: Record<string, unknown> }) => { try { return { data: name === 'sygsphere_files' ? [] : request(args.action, args.input), error: null } } catch (error) { return { data: null, error: { message: error instanceof Error ? error.message : 'Failed' } } } } }
}
