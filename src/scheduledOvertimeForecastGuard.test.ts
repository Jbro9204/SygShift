...
describe('Scheduled overtime forecast guardrails', () => {
  ...
  it('provides a focused UI and audited Excel export without confusing candidates with availability', () => {
    ...
    expect(workspace).toContain('reports-overtime-heading__main')
    expect(workspace).toContain('reports-overtime-filter-grid')
    expect(workspace).toContain('reports-overtime-schedule-context')
    expect(styles).toContain('.reports-overtime-back-link')
    expect(styles).toContain('grid-template-columns: minmax(15rem, 0.8fr) minmax(20rem, 1.55fr) minmax(14rem, 0.7fr)')
