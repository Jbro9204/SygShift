...
- [x] Make detected native and flattened PDF fields directly clickable and editable on the document while keeping the side field list synchronized as a secondary navigation aid.
...
- [x] An authorized user can click a visible text, narrative, choice, checkbox, or signature field on the PDF, complete it in place, and see the same synchronized value in the guided side list.
...
- Status: Released and production-verified 09/11/2026
- Added: 09/09/2026
...
- [x] Create one canonical lifecycle case and timeline for voluntary resignation, involuntary
...
- [x] Carry an approved future separation into a protected effective-date queue. Trigger prominent,
  mandatory HR reminders and provide one clear final execution action from the case and Employee File.
- [x] Before enabling unattended effective-date execution, obtain an explicit management decision on
  whether access shutdown must remain a final human-confirmed action or may execute automatically.
- [x] Preserve independent maker-checker approval: the person who opens a separation case cannot be
  its approver, and the final actor must have recent MFA, `hr.people.manage`, and
  `hr.offboarding.approve`.
- [x] Reuse the canonical termination transaction for final execution: mark the employee Separated,
...
- [x] Add a role-appropriate checklist with owners, due dates, completion evidence, and reasoned
...
- [x] Connect the approved HR forms without copying files or facts: GS-HR-700, GS-HR-701, GS-HR-702,
...
- [x] Keep restricted separation reasons, investigations, medical information, and legal material in
...
- [x] Handle active shifts, open punches, pending corrections, an open payroll period, leave,
...
- [x] Prevent case completion until every required task is complete or an authorized person records a
  waiver and reason. Never silently mark downstream work complete.
- [x] Preserve all historical schedules, punches, timecards, payroll rows, licenses, documents,
  training, cases, approvals, account history, and audit events. Termination must never hard-delete or
  rewrite history.
- [x] Add governed offboarding dashboards and exportable reports for scheduled separations, overdue
  tasks, access shutdown, released shifts, property recovery, documents, final-pay readiness, and
  completed cases.
- [x] Make rehire a separate approved restoration workflow. Do not silently reactivate prior roles,
  permissions, MFA devices, schedules, pay, licenses, or assignments.
- [x] Verify navigation, permissions, Worker endpoints, database enforcement, MFA/FIDO behavior,
...
- Status: Released to a controlled owner canary 09/11/2026; broad enrollment remains acceptance-gated
- Added: 09/01/2026
...
- [x] Trigger the checkpoint from authoritative pending-action records rather than hard-coded pages, banners, or duplicated client state.
...
- [x] Present multiple requirements as one ordered queue with clear progress such as **Action 1 of 3**, prioritizing same-day schedule changes and critical safety items.
- [x] Do not allow closing, skipping, refreshing, signing out and back in, or opening a direct route to bypass outstanding required actions.
- [x] Preserve immediate access to clock in/out, report a call-off, and reach emergency information so the checkpoint cannot make an employee late or obstruct a safety workflow. After the urgent action is handled, return the employee to the required queue before opening the rest of SygShift.
- [x] Use action-specific wording and buttons. Distinguish confirmation, acknowledgment of receipt, employee attestation, acceptance, decline, dispute, and legally binding signature instead of treating them as interchangeable.
- [x] For write-ups or corrective actions, make **Acknowledge receipt** explicitly mean receipt rather than agreement and provide a recorded employee-response or decline path.
- [x] Keep legal electronic signatures in a separately validated signature workflow; an acknowledgment button must never be represented as a signature.
...
- [x] Record the employee, exact content or schedule version, assigned/viewed/completed timestamps, response wording, action taken, resolution source, and session evidence in the existing audit-safe Action Center history.
- [ ] Provide compact permission-scoped manager reporting for pending, completed, overdue, declined, disputed, and unreachable employees without creating long-scroll lists. Pending and overdue reporting is live; completed and unreachable rollups remain for broad-rollout acceptance.
- [x] Support desktop and mobile layouts, keyboard navigation, screen readers, browser zoom, light/dark modes, time-zone-correct schedule details, and clear recovery from network interruption.
- [x] Enforce the checkpoint and completion rules on the server as well as the client so navigation changes or direct API calls cannot bypass the requirement.

Completion criteria:

- [x] A newly assigned required action reliably appears after the employee's next successful login or session restoration.
- [x] The employee cannot enter ordinary SygShift routes until all current required actions are resolved, while clock, call-off, and emergency access remain available.
- [x] Completing, declining, disputing, superseding, or expiring an item updates the active queue exactly once and creates a permanent, readable Action Center History record.
- [x] Authorized managers can identify outstanding and overdue requirements within their permitted scope without gaining access to unrelated employee or HR records.
- [x] Allow, deny, bypass, refresh, retry, multiple-action, superseded-version, time-zone, accessibility, and rollback tests pass before production activation.
...
- Status: Stage 0/1 contract checkpoint and Stage B private tenant/authorization foundation recorded 09/19/2026; all runtime capability flags and role grants remain off
- Added: 09/19/2026
...
- [~] Add SygShift-owned additive tenant, authorization, history, audit, and usage migrations with RLS and allow/deny regression coverage. Private tenant and server authorization foundation are recorded; history, audit, usage, and database regression evidence remain pending.
