...
function CreateOccurrenceDialog({ onClose, onSaved, workspace }: { onClose: () => void; onSaved: () => Promise<void>; workspace: AccountabilityWorkspace }) {
  ...
  const [eventType, setEventType] = useState<AccountabilityEventType>('call_off')
  ...
  const [coverageCallOffId, setCoverageCallOffId] = useState<string | null>(null)
  const mutation = useMutation({ mutationFn: createAccountabilityOccurrence, onSuccess: async (result) => { await onSaved(); if (result.callOffId) setCoverageCallOffId(result.callOffId); else onClose() } })
...
function ClassificationForm({ event, onSaved }: { event: AccountabilityEvent; onSaved: (callOffId: string | null) => Promise<void> }) {
  const [eventType, setEventType] = useState(event.eventType)
  const [reason, setReason] = useState('')
  const mutation = useMutation({ mutationFn: reclassifyAccountabilityOccurrence, onSuccess: async (result) => onSaved(result.coverageRequired ? result.callOffId : null) })
  return <details className="accountability-classification"><summary>Correct occurrence type</summary><form className="time-workflow-form" onSubmit={(submitEvent) => { submitEvent.preventDefault(); mutation.mutate({ eventId: event.id, eventType, reason }) }}>
    <p>Use only to correct a documented fact. The original note and review decision are preserved. Changing this to an absence starts the guided coverage plan; no punch or payroll record is changed.</p>
