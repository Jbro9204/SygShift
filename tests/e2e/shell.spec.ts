...
test('schedule remains usable on the configured viewport', async ({ page }) => {
  await page.goto('/schedule')

  await expect(page.getByRole('heading', { name: 'Schedule', exact: true })).toBeVisible()
  const viewportWidth = page.viewportSize()?.width ?? 0
  if (viewportWidth < 900) {
    await expect(page.getByLabel('Mobile schedule view')).toBeVisible()
  } else {
    await expect(page.getByRole('table', { name: 'Weekly schedule' })).toBeVisible()
