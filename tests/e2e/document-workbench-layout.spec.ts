...
async function expectPeopleListUsable(page: Page) {
  const geometry = await page.locator('.document-workbench__people').evaluate((list) => {
    const box = list.getBoundingClientRect()
    const firstChoice = list.querySelector('label')?.getBoundingClientRect()
    return { choiceHeight: firstChoice?.height ?? 0, height: box.height, scrollHeight: list.scrollHeight }
  })
  expect(geometry.height).toBeGreaterThanOrEqual(90)
  expect(geometry.choiceHeight).toBeGreaterThanOrEqual(50)
...
for (const theme of ['light', 'dark'] as const) {
  ...
  test(`employee-file search stays clear in ${theme} mode`, async ({ page }, testInfo) => {
    ...
    await expectPeopleListUsable(page)
    expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1)
    expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([])
