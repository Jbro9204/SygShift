...
begin
  ...
  for period_index in 0..2 loop
    if not coalesce((historical_period ->> 'resolved')::boolean, false) then
      raise object_not_in_prerequisite_state using message = 'A recent payroll period could not be resolved.';
    end if;
    available_periods := available_periods || jsonb_build_array(jsonb_build_object(
      'offset', period_index, 'fromDate', historical_period ->> 'periodStartsOn', 'throughDate', historical_period ->> 'periodEndsOn'));
    cursor_date := (historical_period ->> 'periodStartsOn')::date - 1;
    historical_period := private.get_payroll_period_for_week(cursor_date - mod(extract(dow from cursor_date)::integer - rules.week_starts_on + 7, 7));
  end loop;
