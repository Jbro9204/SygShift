...
comment on function public.get_weekly_schedule_payload(date) is
  'Returns the latest visible schedule revision for the week. Older drafts cannot hide newer published revisions.';

comment on function public.ensure_schedule_draft(date) is
  'Archives stale older drafts before opening or creating the current working draft from the latest published revision.';
