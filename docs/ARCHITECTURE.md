...
- Reminder definitions and recipient occurrences are private server-authoritative records tied to one task. A reminder fires once; an alarm remains active until its recipient stops, snoozes, completes the task when authorized, or the task/reminder lifecycle cancels it. Opening the task does not silence the alarm.
...
- The SygTasks launcher owns a badge independent from SygSphere. A visible authorized shell displays the active alarm and uses a one-tab lease so multiple tabs do not overlap audio. Each cycle plays the complete custom sound, waits exactly three seconds, and repeats until a terminating lifecycle action; background or closed-browser delivery remains subject to browser and device notification behavior.
- Device sound preferences separate task-alarm volume from ordinary login and notification volume. Scheduling or receiving an active alarm automatically enables nonzero alarm audio on that device, subject to the browser's audio permission. Server acknowledgment and snooze persist across sessions and devices; opening a task neither silences the browser nor records a false acknowledgment.
...
- The normal internal delivery path begins with an outside supported file, automatically selects the compatible general protected vault, accepts one to 25 active employee recipients and an action, waits for the existing malware scan, then applies the approved standard electronic-signature policy and sends the tracked request. Templates, policies, and specialized filing remain available as advanced administration rather than mandatory setup for ordinary proposals and documents.
