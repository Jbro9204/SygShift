...
export function SupportTicketForm({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    const dialog = progressRef.current?.closest('dialog')
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => { await Promise.all([queryClient.invalidateQueries({ queryKey: ['support'] }), queryClient.invalidateQueries({ queryKey: ['my-notifications'] })]) },
  })
  ...
  function submit(event: FormEvent) {
    ...
    if (step !== 4 || !category || !formValid || mutation.isPending) return
    mutation.mutate({
      requestId, category, subcategory, subject: subject.trim(), description: description.trim(),
  ...
  if (mutation.isSuccess) return <ModalDialog className="modal-dialog--support" onClose={onClose} title="Support ticket submitted">
    ...
    <dl className="support-review-grid"><div><dt>Status</dt><dd>New</dd></div><div><dt>Priority</dt><dd>{mutation.data.priority.charAt(0).toUpperCase() + mutation.data.priority.slice(1)}</dd></div><div><dt>Routed to</dt><dd>{routingLabel}</dd></div><div><dt>Updates</dt><dd>In SygShift and by email</dd></div></dl>
  ...
  return <ModalDialog busy={mutation.isPending} busyLabel="Submitting your support ticket…" className="modal-dialog--support" description="Answer the questions below so your request reaches the correct authorized role with the information needed to help." eyebrow={`Step ${step} of 4`} headingIcon={<LifeBuoy size={22} />} onClose={onClose} title="How can we help?">
    <form className="support-form" inert={mutation.isPending} onSubmit={submit}>
      <div ref={progressRef}><SupportProgress label="Request progress" labels={['Topic', 'Details', 'Impact', 'Review']} current={step - 1} /><p className="visually-hidden" role="status">Step {step} of 4: {['Topic', 'Details', 'Impact', 'Review'][step - 1]}</p></div>
