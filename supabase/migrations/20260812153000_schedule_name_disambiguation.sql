...
comment on function public.get_schedule_builder_options() is
  'Returns active scheduling employees with employee numbers so operational selectors can disambiguate similar names.';

comment on function public.get_weekly_schedule_payload(date) is
  'Returns the current weekly schedule with employee numbers for authorized schedule assignment detail.';
