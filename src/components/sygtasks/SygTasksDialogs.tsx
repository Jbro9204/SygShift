...
import { Archive, Bell, BellOff, Check, CheckCircle2, Circle, History, MessageSquare, ShieldAlert, Tag, Users } from 'lucide-react'
...
export function TaskDetailDialog({ workspace, detail, busy, error, referenceTime, canLoadMoreCandidates = false, loadingMoreCandidates = false, onLoadMoreCandidates, onClose, act }: {
  ...
  return (
    <ModalDialog className="sygtasks-detail-dialog" title={detail.title} eyebrow={`${workspace.selectedBoard?.name ?? 'SygTasks'} · ${formatSygTaskStatus(detail.status)}`} description={`${formatSygTaskPriority(detail.priority)} priority · Updated ${formatTaskDue(detail.updatedAt)}`} busy={busy} onClose={onClose}>
      ...
      {section === 'activity' ? <div className="sygtasks-detail-section"><section><h3><History aria-hidden="true" size={18} />Activity</h3><ol className="sygtasks-activity">{detail.activity.map((item) => <li key={item.id}><span aria-hidden="true" /><div><strong>{item.actorName}</strong> {item.action.replaceAll('.', ' ')}<time>{formatTaskDue(item.createdAt)}</time></div></li>)}</ol></section></div> : null}
    </ModalDialog>
  )
}
