...
describe('enterprise Document Studio safeguards', () => {
  ...
  it('ships real management and employee execution surfaces with a PDF renderer', () => {
    ...
    expect(viewer).toContain('new Uint8Array(await response.arrayBuffer())')
    expect(viewer).toContain('new ResizeObserver')
    expect(viewer).toContain('Rendering page {page}')
    expect(viewer).toContain('await previousTask.promise.catch')
  })
})
