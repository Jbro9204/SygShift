...
test('shows reader identity and read time from the Read by control on hover, tap, and keyboard focus', async ({ context }, testInfo) => {
  const scope = crypto.randomUUID(); const sender = await context.newPage(); const reader = await context.newPage()
  await sender.goto(`${fixture}?scope=${scope}`)
  await sender.getByRole('textbox', { name: 'Write a message', exact: true }).fill('Read receipt detail check')
  await sender.getByRole('button', { name: 'Send', exact: true }).click()
  await reader.goto(`${fixture}?scope=${scope}&second`)
  await expect(reader.locator('.sphere-message__body').filter({ hasText: 'Read receipt detail check' })).toBeVisible()

  const receipt = sender.locator('summary[aria-label="Read by 1. Show readers."]')
  await expect(receipt).toBeVisible()
  if (testInfo.project.name === 'mobile-chromium') await receipt.tap()
  else await receipt.hover()
  const panel = sender.locator('.sphere-read-panel')
  await expect(panel).toContainText('Devon Ruiz')
  await expect(panel.locator('time')).toBeVisible()

  await receipt.press('Escape')
