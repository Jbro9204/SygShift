...
describe('SygSphere experience boundaries', () => {
  ...
  it('keeps inline and HR limits intact while larger images use a bounded resumable quarantine path', () => {
    ...
    expect(worker).toContain("if (completion.state && ['rejected', 'error'].includes(completion.state))")
    expect(worker).toContain("if (rejected.objectKey && ['rejected', 'error', 'expired'].includes(rejected.state ?? ''))")
