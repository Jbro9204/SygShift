...
for (const theme of ['light', 'dark'] as const) {
  test(`PDF workbench text editing is contained and readable in ${theme} mode`, async ({ page }, testInfo) => {
    ...
    await expect(directPosition).toHaveValue('Business Development Engineer')
    const directLongAnswer = page.getByRole('textbox', { name: 'Employee explanation on document' })
    const directFieldGeometry = await directLongAnswer.evaluate((control) => {
      const field = control.getBoundingClientRect()
      const sheet = control.closest('.document-workbench__sheet')!.getBoundingClientRect()
      return {
        bottomInsideSheet: field.bottom <= sheet.bottom + 1,
        overflowY: getComputedStyle(control).overflowY,
        fontFamily: getComputedStyle(control).fontFamily,
      }
    })
    expect(directFieldGeometry.bottomInsideSheet).toBe(true)
    expect(directFieldGeometry.overflowY).toBe('auto')
    expect(directFieldGeometry.fontFamily.toLocaleLowerCase()).not.toContain('monospace')
