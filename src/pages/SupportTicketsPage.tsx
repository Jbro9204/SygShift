...
const statusLabel = (value: string) => categoryLabel(value)
...
export function SupportTicketsPage() {
  ...
  const refresh = async () => Promise.all([queryClient.invalidateQueries({ queryKey: ['support', 'workspace'] }), queryClient.invalidateQueries({ queryKey: ['support', 'ticket', selectedId] })])
  const replyMutation = useMutation({ mutationFn: () => addSupportTicketMessage(selectedId!, reply.trim(), internal), onSuccess: async () => { setReply(''); setInternal(false); await refresh() } })
  ...
  return <div className="page page--support support-workspace">
    ...
    {!isSupabaseConfigured ? <DataStatePanel icon={LifeBuoy} title="Support needs the secure connection" tone="setup"><p>Connect the protected data service before submitting or reviewing tickets.</p></DataStatePanel> : workspaceQuery.isPending ? <DataStatePanel icon={Inbox} title="Loading support tickets"><p>Checking your permitted ticket queues.</p></DataStatePanel> : workspaceQuery.isError ? <DataStatePanel icon={LifeBuoy} title="Support tickets unavailable" tone="error"><p>{workspaceQuery.error.message}</p></DataStatePanel> : workspace ? <>
      ...
      <section className="support-board">
        <div className="support-list-panel"><div className="support-list-toolbar"><label className="support-search"><span>Search tickets</span><div><Search size={18} /><input onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Ticket number or subject" value={search} /></div></label><label><span>Status</span><select onChange={(event) => { setStatus(event.target.value); setPage(1) }} value={status}><option value="open">Open tickets</option><option value="new">New</option><option value="in_progress">In progress</option><option value="waiting_on_employee">Waiting on employee</option><option value="resolved">Resolved</option><option value="closed">Closed</option><option value="all">All tickets</option></select></label></div>
          {workspace.tickets.length === 0 ? <DataStatePanel icon={Inbox} title="No support tickets match"><p>Change the filters or submit a new request.</p></DataStatePanel> : <div className="support-ticket-list">{workspace.tickets.map((item) => <button className={selectedId === item.id ? 'support-ticket-row selected' : 'support-ticket-row'} key={item.id} onClick={() => setParams({ ticket: item.id })} type="button"><div><span>{item.ticketNumber}</span>{item.confidential ? <LockKeyhole aria-label="Confidential" size={14} /> : null}<span className={`support-priority support-priority--${item.priority}`}>{item.priority}</span></div><strong>{item.subject}</strong><small>{categoryLabel(item.category)} · {item.submittedBy.name}</small><footer><span className={statusClass(item.status)}>{statusLabel(item.status)}</span><time>{formatOperationalDateTime(item.updatedAt)}</time></footer></button>)}</div>}
          <div className="communications-pagination"><span>Page {workspace.page.number} of {workspace.page.totalPages} · {workspace.page.total} tickets</span><label><span>Rows</span><select onChange={(event) => { setPageSize(Number(event.target.value) as 5 | 10 | 20); setPage(1) }} value={pageSize}><option value={5}>5</option><option value={10}>10</option><option value={20}>20</option></select></label><button className="secondary-button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} type="button">Previous</button><button className="secondary-button" disabled={page >= workspace.page.totalPages} onClick={() => setPage((value) => value + 1)} type="button">Next</button></div>
        </div>
        <div className="support-detail-panel">{!selectedId ? <DataStatePanel icon={MessageSquare} title="Choose a ticket"><p>Select a ticket to review its conversation and linked context.</p></DataStatePanel> : detailQuery.isPending ? <DataStatePanel icon={MessageSquare} title="Loading ticket"><p>Loading the complete conversation.</p></DataStatePanel> : detailQuery.isError ? <DataStatePanel icon={LifeBuoy} title="Ticket unavailable" tone="error"><p>{detailQuery.error.message}</p></DataStatePanel> : ticket ? <>
          ...
          {ticket.canManage ? <section className="support-management" aria-label="Ticket management"><label><span>Status</span><select disabled={updateMutation.isPending} onChange={(event) => updateMutation.mutate({ status: event.target.value as SupportTicketStatus })} value={ticket.status}><option value="new">New</option><option value="assigned">Assigned</option><option value="in_progress">In progress</option><option value="waiting_on_employee">Waiting on employee</option><option value="resolved">Resolved</option><option value="closed">Closed</option><option value="reopened">Reopened</option></select></label><label><span>Priority</span><select disabled={updateMutation.isPending} onChange={(event) => updateMutation.mutate({ priority: event.target.value as SupportTicketPriority })} value={ticket.priority}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><label><span>Assigned to</span><select disabled={updateMutation.isPending || assigneesQuery.isPending} onChange={(event) => updateMutation.mutate({ assignedTo: event.target.value || null })} value={ticket.assignedTo?.id ?? ''}><option value="">Unassigned</option>{assigneesQuery.data?.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label></section> : null}
          ...
          <form className="support-reply" onSubmit={submitReply}><label><span>{internal ? 'Internal note' : 'Reply to ticket'}</span><textarea maxLength={5000} onChange={(event) => setReply(event.target.value)} placeholder={internal ? 'Visible only to authorized ticket handlers…' : 'Write a response. A system-generated email will be sent automatically…'} rows={5} value={reply} /></label>{ticket.canManage ? <label className="support-check"><input checked={internal} onChange={(event) => setInternal(event.target.checked)} type="checkbox" /><span>Add as an internal note—do not email the requester</span></label> : null}{replyMutation.isError ? <div className="inline-alert" role="alert">{replyMutation.error.message}</div> : null}{replyMutation.isSuccess ? <div className="inline-success"><CheckCircle2 size={18} />Message added and lifecycle notifications queued.</div> : null}<div className="support-reply-actions"><span>{internal ? <><ShieldCheck size={16} />Authorized staff only</> : <><Send size={16} />In-app and email update</>}</span><button className="primary-action" disabled={!reply.trim() || replyMutation.isPending} type="submit">{replyMutation.isPending ? 'Sending…' : internal ? 'Add internal note' : 'Send reply'}</button></div></form>
