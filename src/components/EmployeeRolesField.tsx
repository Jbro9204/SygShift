  ...
  return (
    <fieldset className={`employee-roles${expanded ? ' is-expanded' : ' is-collapsed'}`}>
      ...
      <div className="employee-roles__summary">
        <div>
          <strong>{draft.ids.length} {draft.ids.length === 1 ? 'role' : 'roles'} assigned</strong>
          <span>{draft.primaryRole ? `${employeeRoleLabels[draft.primaryRole]} sets scheduling and timekeeping.` : 'A scheduling and timekeeping role is required.'}</span>
        </div>
      ...
      {expanded ? <div className="employee-roles__content" id={`${id}-content`}>
        ...
        <div className="employee-roles__toolbar">
          <label className="employee-roles__search">
            ...
            <div><Search aria-hidden="true" size={18} /><input disabled={disabled || unavailable} onChange={(event) => { event.stopPropagation(); setSearch(event.target.value) }} placeholder="Search by role name" type="search" value={search} /></div>
          </label>
          ...
          {visible.map((role) => {
            ...
            return (
              <div className={`employee-roles__row${selected ? ' is-selected' : ''}`} key={role.id}>
                <label>
                  <input aria-label={presentation.name} checked={selected} disabled={disabled || unavailable || (!role.active && !selected) || (!canManage && role.baseAppRole === 'admin')}
                ...
                {canManage && selected && role.active && role.systemRole && role.baseAppRole && !isPrimary ? (
                  <button aria-label={`Use ${presentation.name} for scheduling and timekeeping`} className="secondary-button secondary-button--small" disabled={disabled || unavailable} onClick={() => onChange({ ...draft, primaryRole: role.baseAppRole })} type="button">Make default</button>
                ) : null}
              </div>
            )
          })}
