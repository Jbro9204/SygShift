...
-- A direct-audio offer is valid only while the caller is still an active
-- member of the exact direct conversation that created the call. This
-- replaces the prior generic media.ready branch, which correctly checked the
-- account permission but had no fresh relationship scope to validate.
...
begin
  ...
  if target_command_kind in (
    ...
    scope_verified := true;
  elsif target_command_kind in ('call.request', 'meeting.create', 'media.ready') then
